...
export default function FintechCard12() {
  ...
  // 최신 환율과 변동률 계산 (CNH는 대문자)
  const currentRate = exchangeData[0]?.CNH || 0
  const previousRate = exchangeData[1]?.CNH || currentRate
  ...
  const chartValues = exchangeData.slice(0, 21).reverse().map(item => item?.CNH || 0)
      ...
      <div className="px-5 pt-5">
        <header>
          <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">
            <span className="text-gray-800 dark:text-gray-100">CNY</span> - 중국 위안화
          </h3>
