      ...
      <div className="px-5 pt-5">
        <header>
          <h3 className="text-sm font-semibold text-gray-500 uppercase mb-1">
            <span className="text-gray-800 dark:text-gray-100">{currencySymbol}</span> - {currencyName}
          </h3>
          ...
          <div className="text-sm">
            <span className={`font-medium ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
              {isPositive ? '+' : ''}₩{Math.abs(change).toFixed(2)} ({isPositive ? '+' : '-'}{Math.abs(Number(changePercent)).toFixed(2)}%)
            </span> - Today
          </div>
        </header>
      </div>
