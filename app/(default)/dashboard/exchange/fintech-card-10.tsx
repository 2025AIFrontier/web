...
export default function FintechCard10() {
  ...
  // 최신 환율과 변동률 계산 (USD는 대문자)
  const currentRate = exchangeData[0]?.USD || 0
  const previousRate = exchangeData[1]?.USD || currentRate
  ...
  const chartValues = exchangeData.slice(0, 21).reverse().map(item => item?.USD || 0)
      ...
      <div className="px-5 pt-5">
        <header>
          <h3 className="text-sm font-semibold text-gray-500 uppercase mb-1">
            <span className="text-gray-800 dark:text-gray-100">USD</span> - 미국 달러
          </h3>
