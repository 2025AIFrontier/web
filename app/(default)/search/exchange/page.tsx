...
export default function Exchange() {
  ...
  // 차트 데이터 준비 (최근 2주)
  const prepareChartData = (history: ExchangeData['history'], currency: 'usd' | 'eur' | 'cny' | 'jpy100') => {
    if (!history || history.length === 0) return { data: [], labels: [] }
    
    // 최근 14일 데이터만 추출
    const recentData = history.slice(-14)
    return {
      data: recentData.map(item => item[currency] || 0),
  ...
  const stockData = [
    ...
    },
    {
      ticker: 'JPY100',
      company: '일본 엔(100)',
      price: `₩${(exchangeData.jpy100 || 0).toFixed(2)}`,
      ...calculateChange(exchangeData.jpy100 || 0, exchangeData.previousJpy100),
      ...prepareChartData(exchangeData.history, 'jpy100'),
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <ExchangeCardBottom2 exchangeData={exchangeData} />
